...
The accessibility audit also surfaced real WCAG 2.1 AA violations on every page, and this is where things got properly interesting. Running axe locally showed every failure was inside DOM injected at *runtime by third parties* — the [klaro](https://klaro.org/) cookie banner loaded via GTM, and the pagefind search input. Specifically: the klaro banner's muted-grey body text and green-on-#333 link colour both failed 4.5:1 contrast, the green accept buttons failed 3:1, the privacy-policy link inside the body text failed `link-in-text-block` because it had nothing but colour to distinguish it from the surrounding text, and the pagefind input was using `title` instead of `aria-label`. None of those were in *my* code — they were in DOM injected by a third-party widget — but the test holds you responsible for them anyway, which is exactly right. Fixed with: `!important` CSS overrides scoped to `.klaro *` (the runtime-injected stylesheet wins on load order otherwise), `text-decoration: underline` on klaro's links, and a post-init `aria-label` on pagefind's input. Six lines of JS, twenty lines of CSS, zero serious/critical violations across `/`, `/about`, and `/blog`. The check was honest; the fix was honest; the site is materially more usable than it was this morning.
